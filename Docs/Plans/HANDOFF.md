...
**Update (v2.1, after Ryan's play test) — spawn-on-tree bug fixed + forest thinned:** Ryan spawned ON a canopy and was stuck. Root cause: the NavMesh bakes from RENDER meshes — canopy tops baked as walkable islands, AND the outer NotWalkable plane sat 3cm under the meadow so the voxelizer merged them and poisoned real floor; the agent snapped to the nearest surviving walkable surface: a treetop. Fix: ALL decorative dressing (canopies, branches, grass, flowers, mushrooms, outer plane) now carries `NavMeshModifier.ignoreFromBuild` — it can never shape the mesh; only trunks + the monolith still block. Boundary is now the meadow plane's own edge (agents can't leave the mesh). Trees thinned per Ryan: treeline 32→21, field trees 4→3. **Lesson for every future biome: dressing must be bake-inert by default — mark it ignored unless it's MEANT to block.** Batchmode verified — 0 errors, 0 warnings.
